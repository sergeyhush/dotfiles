...
hyper.start = function(config_table)
    local function showHyperBindings()
        local lines = {}
        for name, app in pairs(config_table.applications) do
            if app.hyper_key then
                table.insert(lines, string.format("%s → %s", app.hyper_key, name))
            end
        end
        table.sort(lines)
        if #lines == 0 then
            hs.alert.show("No Hyper app bindings", 1.5)
        else
